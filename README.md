...
|     └──────────────┴──────────────┴──────────────┴─────────────┘   |
|                  接收采集/快速分发 s_rx_window_step()（非阻塞）       |
+---------------------------------^----------------------------------+
                                  | 只调 adx_port 接口
+---------------------------------|----------------------------------+
...
当前实现会遍历全部已注册 URC 回调，某个回调返回 `ADX_OK` 后仍继续通知后续回调。同一聚合窗口随后还会交给当前在途指令的 `rx_cb`，因此 URC 既能处理模组主动上报，也能作为主动 AT 指令响应的兜底处理路径。注意：URC 命中本身不会自动结束当前 AT 事务。

### 4.7 接收窗口聚合（非阻塞状态机）

UART 新数据先写入独立采集缓冲区，与当前 AT 事务的响应累积缓冲区分离，避免下一轮接收覆盖正在解析的数据。以 `\r\n` 结尾的文本数据会立即分发；没有明确文本边界的数据才使用 `ADX_RX_WINDOW_MS`（默认 20ms）帧间静默时间兜底。当前 AT 回调只在有新数据时执行，并可跨多个分片查看累积响应。

RX 读取和 URC 分发位于状态机节奏控制之前，因此 `RESP_OK` 冷却期间仍会持续接收和处理模组数据；冷却只限制下一条 AT 指令的发送。
...
| `ADX_RX_WINDOW_MS`             | `20`   | 无明确文本边界时的帧间静默兜底时间（ms）                         |
...
| `ADX_RESP_OK_COOLDOWN_MS`      | `50`   | RESP_OK 后限制下一条发送的冷却时间，不暂停 RX/URC          |
