...
/*
 * 非文本/不完整数据的帧间静默兜底时间(ms)。
 * 以CRLF结尾的文本帧会立即分发，不等待该时间；只有尚无明确文本边界的数据
 * 才在持续无新分片达到此时间后分发。默认20ms，避免旧版320ms固定延迟。
 */
#ifndef ADX_RX_WINDOW_MS
#define ADX_RX_WINDOW_MS (20U)
#endif
 ...
 *
 *   ADX_RESP_OK_COOLDOWN_MS  - RESP_OK后限制下一条发送的冷却时间；不暂停RX/URC
